...
***注：現在サポートしている音声ファイルの形式は`.wav, .WAV`ファイルと`.mp3`ファイルです***。*`.mp3`ファイルは対応する`.wav`ファイルに変換・保存されます*。
...
また、下のパネルの「オーディオ設定」セクションで、音声ファイルが複数ある場合の音声間の無音時間、「アニメーション設定」セクションで目の動きの変更や、定期往復アニメーション時の往復時間、口パクの大きさを設定できます（口パクを大きくしたい場合、口パクのしきい値に対する倍率を小さくしてください）。
